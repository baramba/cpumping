...
void setup()  {
  ...
  Serial.begin(BAUDRATE);                                                   // Инициализируем вывод данных на серийный порт со скоростью 9600 бод

  WiFi.begin(ssid, password);                                                    // Создаём точку доступа
  
  bool fsok = LittleFS.begin();
  Serial.printf_P(PSTR("FS init: %s\n"), fsok ? PSTR("ok") : PSTR("fail!"));                                               // Инициализируем работу с файловой системой                          
...
// Обработка HTTP-запросов
  ...
  if(!handleFileRead(HTTP.uri()))                                       // Если функция handleFileRead (описана ниже) возвращает значение false в ответ на поиск файла в файловой системе
      HTTP.send(404, "text/plain", "Not found - "+HTTP.uri());                        // возвращаем на запрос текстовое сообщение "File isn't found" с кодом 404 (не найдено)
  });
}
